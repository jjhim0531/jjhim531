<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
	...
	<body>
		...
		<!-- HTML주석 : 개발자도구 탭에 노출됨
		why? html은 브라우저의 언어이기에 요청이 톰캣서버에 가서도 읽히지 않고 살아있다가 브라우저로 다시 돌아오면 읽혀지기에 브라우저에 보이게 됨-->

		<%-- JSP주석 : 개발자도구 탭에 노출안됨
		why? java는 서버에서 실행되기에 요청이 톰캣서버에 도착했을때 주석이 읽혀지고 이미 처리됫다고 봐야함. 그러므로 브라우저에서 다시 와서는 보이지 않음 --%>

		<%
		//스크립틀릿 : 해당영역에 일반적인 자바코드를 작성(변수선언 및 초기화, 제어문 등)
		//(영역 : 이 페이지 내에서는 전부 같은 변수를 공유하므로 이 페이지 내에서는 계속 살아있다.)
		int sum = 0;
		for(int i = 1; i <= 100; i++) {
			sum += i;
		}
	...
	<!--
		...
		* out.println(): JSP에서 HTML 코드로 변환되어 브라우저에 출력.
	-->
