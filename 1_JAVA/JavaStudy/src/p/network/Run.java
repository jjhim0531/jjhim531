...
/*
 	...
 	포트(Port) : 같은 컴퓨터 내에서 프로그램을 식별하는 번호.(어떤 주소지에 도착하더라도 어떤 방 문을 열지 알아야함.)
 				클라이언트는 서버연결요청 시 IP주소, 포트번호 모두 알아야함.
 	...
 	* 현재 구동중인 서버가 있을 때
 	  ...
 	  *자바만 가지고 서버와 클라이언트의 간단한 통신을 해보자
 	   ...
 	   - TCP방식 : 데이터의 전송속도가 느려! 데이터가 정확하고 안정적으로 전달이 가능합니다(신뢰성이 요구되는 프로그램)(연결지향적 프로토콜)
 	   - UDP방식 : 데이터의 전송속도가 빠름! 신뢰성이 없는 데이터 전달에 용이(데이터를 빠른속도로 전송하고자할 때)(비연결 지향적 프로토콜)
 */
