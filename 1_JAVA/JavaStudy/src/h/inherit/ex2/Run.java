...
public class Run {
	 ...
	 * - 부모클래스에 정의되어있는 protected 필드는 자식클래스에서 직접접근이 가능하다(private은 불가)
	 * - 자식객체는 부모클래스에 있는 메소드를 마치 내 것처럼 호출할 수 있다.
	 ...
	 * -> 오버라이딩도 가능하다.
	 * 
	 * cf) static은 원래 1개이므로 상속 개념이 없다.그러ㅁ므로 상속은 안된다. 그러나 어디서든 접근해서 쓸수 있다.
	 */
