...
public class Operator4 {
	 ...
	 * == 연산자는 **객체의 주소값(참조값)**을 비교한다.
	 * . 따라서, 문자열의 내용이 동일하더라도, 서로 다른 객체라면 == 연산자는 false를 반환한다.
	 * 
	 * 이유는 참조변수(객체를 담는 변수)이기 때문에(실제 문자열 값이 아니라, 문자열 객체가 저장된 메모리 주소(참조 값)가 저장)
	 * 
	 * String인 두 객체의 값(내용) 비교시에는
	 * 문자열.equals(문자열);
	 */
